...
f.cumuvstemp.sample <- function(N, f.seed) {
  ...
  rho=.8
  COR<-diag(4)
  COR[2,4]=COR[4,2]=rho;  # specificeer de correlaties; als voorbeeld heb ik een correlatie tussen variabele 2 en 4 (T2010 en CO22010) genomen die gelijk is aan 0.8.
  
  x.cor <-pse::LHScorcorr(x, COR = COR, eps = 0.01, maxIt = 200)    # Pas de sampling aan zodat de correlaties de gewenste waardes hebben
  cor(x.cor)  # check de correlaties
  
  x<-x.cor      # ken de gecorrigeerde waarden weer toe aan x
